module elementary
  ...
  !-- contains :
  ! Heaviside, AngleModulo, ThetaMean
  ! f1, f2, f1_inv, g1, g1_inv
  ! init_rand_seed
  ! poly_abs
...
contains
  ...
  function AngleVec_N(u,v)
    !- compute the angle of a N vector (u,v)
    implicit none
    Double Precision, Dimension(:), intent(in)  :: u,v
    Integer                                     :: i,n
    Double Precision, Dimension(:), Allocatable :: AngleVec_N
    !- the function
    !- init
    n = size(u)
    Allocate(AngleVec_N(n))
    !- loop
    Do i=1,n
       AngleVec_N(i) = AngleVec( (/ u(i), v(i) /) )
    end Do
  endfunction AngleVec_N
  ...
  function g1_inv(s)
    ...
    Double Precision, intent(in)   :: s
    Double Precision, PARAMETER    :: PI = 3.14159265358979323846  
    Double Precision               :: g1_inv
    !-- the function
    g1_inv = 2d0*atan(exp(-s))-PI/2;
  end Function g1_inv
